...
START_TEST(test_config_yaml_parse)
{
    ...
    for (i=0; i<2; i++) {
        ...
        config_opt_add(cp, c, "address", PARAM_STRING, NULL);
        config_opt_add(cp, c, "port", PARAM_INT, NULL);
        config_opt_set_range_int(cp, c, "port", 1, 65535);
        config_opt_add(cp, c, "test_list", PARAM_STRING, NULL);
        config_opt_add(cp, c, "test_list_inline", PARAM_INT, NULL);
        config_opt_add(cp, c, "test_bool", PARAM_BOOL, NULL);
...
{
    config_opt_add(conf, "core", "address", PARAM_STRING, NULL);
