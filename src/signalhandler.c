          ...
          const void *data)
{
  ...
  if (signal_handler->callbacks[siginfo.ssi_signo] != NULL)
    signal_handler->callbacks[siginfo.ssi_signo](signal_handler, signal_handler->data[siginfo.ssi_signo]);
...
static int
signal_handler_set_signal_callback(struct SignalHandler *signal_handler,
                                   unsigned              sig,
                                   ...
                                   void                 *data)
{
  ...
  return signal_handler_set_signal_callback(signal_handler, sig, callback, data);
}
                                      ...
                                      unsigned              sig)
{
  ...
  return signal_handler_set_signal_callback(signal_handler, sig, NULL, NULL);
}
